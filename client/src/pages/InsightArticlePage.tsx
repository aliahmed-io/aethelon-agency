    ...
    <div className="article-content">
      <div className="article-content-main">
        {article.sections.map((section, index) => (
          <div key={section.heading}>
            <section>
              <span className="article-kicker">{article.index} / chapter {String(index + 1).padStart(2, "0")}</span>
              <h2>{section.heading}</h2>
              {section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
            </section>
            {index === 0 && (
              <aside className="article-pull">
                <span className="article-kicker">Studio signal</span>
                <strong>{article.deck}</strong>
              </aside>
            )}
          </div>
        ))}
        <section className="article-proof">
          <span className="article-kicker">Implementation lens</span>
          <h2>What this looks like in a real build.</h2>
          <ul>
            {article.proof.map((item) => (
      ...
      <aside className="article-sidebar">
        <div>
          ...
          <p>{article.statLabel}</p>
          <small>{article.statSource}</small>
        </div>
        <div>
          <span>Topics</span>
          <div className="article-tags">
            {article.tags.map((tag) => <b key={tag}>{tag}</b>)}
          </div>
        </div>
        <div className="article-references">
          ...
          {article.sources.map((source, index) => (
            <a href={source.url} target="_blank" rel="noreferrer" key={source.url}>
              [{index + 1}] {source.label} <ArrowUpRight size={13} />
            </a>
          ))}
        </div>
      </aside>
